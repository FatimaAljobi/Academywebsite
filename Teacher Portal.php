...
die("Connection Failed : ". $conn->connect_error);
  ...
  $Password=mysqli_real_escape_string($conn,$_POST['Passwo']) ;

   $stmt = $conn->prepare("select * from uia_teacher where TeacherName = ? && TeacherID =? "); //ربط الاستاذ
  ...
  {$data =$stmt_result ->fetch_assoc();
      if($data['TeacherID']==="UIA21203902")
      {
        header("location:Admin Dashbord/index.php");
      }
      elseif($data['TeacherID']===$Password)
      {
...
<body style= "background-color: #F1F1F4;" >
    ...
    <div class="container w-50 mt-5">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" style="border: 2px solid rgb(102, 99, 99); border-radius: 20px; padding:30px;">
            <!-- <u style="color: lightslategrey; height:1px;"></u> -->
            <h2 style="text-align: center; font-weight: bold;  font-family: Georgia, Times, serif; color: rgb(218, 11, 11);"> Teacher Portal</h2>
        ...
        <div class="d-grid gap-2 col-8 mx-auto" style="font-weight: bold; font-family: Georgia, Times, serif;">
          <a href="Teacher Page.php"> <button class="btn" style="width:100%; background: #050569; color:#FFFFFF; border:2px solid #aa9b9b; border-radius: 20px; padding: 10px; ">Login</button></a>
        </div>
